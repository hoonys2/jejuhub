...
export async function fetchLiveFlights(customApiKey?: string): Promise<FetchFlightsResult> {
  ...
  try {
    ...
    if (result.status === 'SUCCESS' && Array.isArray(result.data) && result.data.length > 0) {
      ...
      const parsedFlights: FlightItem[] = result.data.map((item: any, idx: number) => {
        ...
        if (status === 'CANCELLED') {
          ...
          // If in flight, calculate realistic elapsed progress
        ...
        // Aircraft type estimate by airline
    ...
    console.warn('Live KAC flight API unavailable, using simulation engine:', err.message);
  }
  ...
  return {
    ...
    message: '공공데이터포털 KAC API 키를 설정하면 실제 운항정보가 연동됩니다.',
  };
}
